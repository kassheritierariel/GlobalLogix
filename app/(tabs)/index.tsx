      ...
      <FlatList
          ...
          <View>
            ...
            <View style={styles.metricsGrid}>
              {metrics.map((metric) => <View key={metric.label} style={styles.metricCard}>
                <View style={[styles.metricIcon, metric.tone === "red" ? styles.metricIconRed : metric.tone === "green" ? styles.metricIconGreen : styles.metricIconBlue]}><MaterialIcon name={metric.icon} size={18} color={metric.tone === "red" ? "#C43D3D" : metric.tone === "green" ? "#147A46" : "#235B9D"} /></View>
            ...
            <View style={transportStyles.block}>
              ...
              <View style={transportStyles.row}>
                {transportFilters.map((filter) => <Pressable key={filter.id} onPress={() => { haptic.selection(); setTransportFilter(filter.id); }} style={({ pressed }) => [transportStyles.chip, transportFilter === filter.id && transportStyles.chipActive, pressed && styles.pressed]}><MaterialIcon name={filter.icon} size={15} color={transportFilter === filter.id ? "#FFFFFF" : "#235B9D"} /><Text style={[transportStyles.text, transportFilter === filter.id && transportStyles.textActive]}>{filter.label}</Text></Pressable>)}
              </View>
            </View>

            <View style={simulationStyles.panel}>
              <View style={simulationStyles.panelHeading}><MaterialIcon name="my-location" size={17} color="#007FFF" /><View style={simulationStyles.panelCopy}><Text style={simulationStyles.panelTitle}>SIMULATION GPS LOCALE</Text><Text style={simulationStyles.panelSubtitle}>Aucune donnée n’est envoyée au transporteur ni au serveur.</Text></View></View>
              <Text style={simulationStyles.fieldLabel}>Type de transport simulé</Text>
              <View style={simulationStyles.chips}>{simulationModes.map((mode) => <Pressable key={mode.id} disabled={simulationRunning} onPress={() => { haptic.selection(); setSimulationMode(mode.id); }} style={({ pressed }) => [simulationStyles.chip, simulationMode === mode.id && simulationStyles.chipActive, simulationRunning && simulationStyles.controlDisabled, pressed && styles.pressed]}><MaterialIcon name={mode.icon} size={14} color={simulationMode === mode.id ? "#FFFFFF" : "#062B5C"} /><Text style={[simulationStyles.chipText, simulationMode === mode.id && simulationStyles.chipTextActive]}>{mode.label}</Text></Pressable>)}</View>
              ...
              <View style={simulationStyles.progressBlock}><View style={simulationStyles.progressMeta}><Text style={simulationStyles.progressLabel}>{simulationRunning ? `${selectedSimulationMode.label} · vitesse ${selectedSimulationSpeed.label.toLowerCase()}` : "Prêt pour une simulation locale"}</Text><Text style={simulationStyles.progressValue}>{simulationRunning ? `${simulationProgress}%` : "0%"}</Text></View><View style={simulationStyles.progressTrack}><View style={[simulationStyles.progressFill, { width: `${simulationProgress}%` }]} />{simulationRunning ? <View style={[simulationStyles.vehicle, { left: `${Math.min(88, simulationProgress * 0.88)}%` }]}><MaterialIcon name={selectedSimulationMode.icon} size={16} color="#062B5C" /></View> : null}</View></View>
            </View>

            <View style={exportStyles.row}><Text style={exportStyles.label}>{filteredShipments.length} colis filtré{filteredShipments.length > 1 ? "s" : ""}</Text><View style={exportStyles.actions}><Pressable disabled={exporting !== null} onPress={() => void runExport("csv")} style={({ pressed }) => [exportStyles.button, pressed && styles.pressed, exporting !== null && exportStyles.disabled]}><MaterialIcon name="table-chart" size={15} color="#235B9D" /><Text style={exportStyles.csvText}>{exporting === "csv" ? "Export…" : "CSV"}</Text></Pressable><Pressable disabled={exporting !== null} onPress={() => void runExport("pdf")} style={({ pressed }) => [exportStyles.button, pressed && styles.pressed, exporting !== null && exportStyles.disabled]}><MaterialIcon name="picture-as-pdf" size={15} color="#C43D3D" /><Text style={exportStyles.pdfText}>{exporting === "pdf" ? "Export…" : "PDF"}</Text></Pressable></View></View>

            <View style={styles.quickActions}>
              <Pressable onPress={() => router.push("/shipments" as never)} style={({ pressed }) => [styles.quickAction, pressed && styles.pressed]}><MaterialIcon name="search" size={19} color="#0A2540" /><Text style={styles.quickActionText}>Rechercher</Text></Pressable>
              <Pressable onPress={() => router.push("/exceptions" as never)} style={({ pressed }) => [styles.quickAction, styles.quickActionAlert, pressed && styles.pressed]}><MaterialIcon name="warning-amber" size={19} color="#C43D3D" /><Text style={[styles.quickActionText, styles.quickActionAlertText]}>Exceptions</Text></Pressable>
              <Pressable disabled={simulationRunning} onPress={runGpsSimulation} style={({ pressed }) => [styles.quickAction, simulationStyles.quickAction, simulationRunning && simulationStyles.controlDisabled, pressed && styles.pressed]}><MaterialIcon name="my-location" size={19} color="#007FFF" /><Text style={styles.quickActionText}>{simulationRunning ? "Simulation…" : "Lancer GPS"}</Text></Pressable>
            </View>
            ...
            {operational.priority ? <Pressable onPress={() => openTracking(operational.priority!)} style={({ pressed }) => [styles.priorityCard, pressed && styles.pressed]}>
              <View style={styles.priorityHeading}><View style={styles.priorityLabel}><MaterialIcon name={priorityOf(operational.priority) === "critical" ? "priority-high" : "radar"} size={16} color="#FFB39A" /><Text style={styles.priorityEyebrow}>{priorityOf(operational.priority) === "critical" ? "INTERVENTION PRIORITAIRE" : "EXPÉDITION À SUIVRE"}</Text></View><MaterialIcon name="arrow-forward" size={22} color="#FFFFFF" /></View>
              <Text style={styles.priorityTitle}>{operational.priority.trackingNumber}</Text>
              <View style={styles.routeRow}><Text numberOfLines={1} style={styles.routeText}>{operational.priority.origin}</Text><MaterialIcon name="trending-flat" size={18} color="#FF6B35" /><Text numberOfLines={1} style={styles.routeText}>{operational.priority.destination}</Text></View>
        ...
        renderItem={({ item }) => {
          ...
          return <Pressable onPress={() => openTracking(item)} style={({ pressed }) => [styles.shipmentRow, priority === "critical" && styles.shipmentCritical, pressed && styles.rowPressed]}>
            <View style={[styles.shipmentIcon, priority === "critical" && styles.shipmentIconCritical]}><MaterialIcon name={item.mode === "air" ? "flight" : item.mode === "sea" ? "directions-boat" : "local-shipping"} size={20} color={priority === "critical" ? "#C43D3D" : "#0A2540"} /></View>
