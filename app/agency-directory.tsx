...
export default function AgencyDirectoryScreen() {
  ...
  if (user?.role !== "super_admin") return <ScreenContainer><View style={styles.center}><MaterialIcon name="lock" size={34} color="#CE1126" /><Text style={styles.title}>Accès éditeur requis</Text><Text style={styles.denied}>L’onboarding d’une nouvelle agence est réservé à GlobalLogix.</Text></View></ScreenContainer>;
  ...
  return <ScreenContainer><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Pressable onPress={() => router.back()} style={styles.back}><MaterialIcon name="arrow-back" size={20} color="#062B5C" /><Text style={styles.backText}>Réglages</Text></Pressable><Text style={styles.eyebrow}>ÉDITEUR GLOBALLOGIX</Text><Text style={styles.title}>Onboarder une agence SaaS</Text><Text style={styles.subtitle}>Créez l’espace isolé de l’agence, désignez son administrateur et générez son lien client personnalisé. L’administrateur reçoit ensuite une invitation et complète le logo, les couleurs et sa connexion WhatsApp.</Text>
    ...
    <View style={styles.step}><MaterialIcon name="link" size={18} color="#007FFF" /><Text style={styles.stepText}>2. Lien client et identité initiale</Text></View>
    ...
    <Pressable disabled={saving} onPress={() => void createAgency()} style={({ pressed }) => [styles.button, saving && styles.disabled, pressed && styles.pressed]}>{saving ? <ActivityIndicator color="#FFFFFF" /> : <><MaterialIcon name="add-business" size={19} color="#FFFFFF" /><Text style={styles.buttonText}>Créer l’espace et inviter l’administrateur</Text></>}</Pressable>
    {message ? <View style={[styles.message, result ? styles.successMessage : styles.errorMessage]}><MaterialIcon name={result ? "check-circle" : "error-outline"} size={17} color={result ? "#147A46" : "#B4232B"} /><Text style={[styles.messageText, { color: result ? "#147A46" : "#B4232B" }]}>{message}</Text></View> : null}
