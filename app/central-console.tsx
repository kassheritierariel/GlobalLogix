...
export default function CentralConsoleScreen() {
  ...
  if (user?.role !== "super_admin") {
    return <ScreenContainer><View style={styles.denied}><MaterialIcon name="lock" size={34} color="#CE1126" /><Text style={styles.deniedTitle}>Accès éditeur requis</Text><Text style={styles.deniedText}>La console centralisatrice est réservée au super administrateur GlobalLogix.</Text></View></ScreenContainer>;
  }
  ...
  return <ScreenContainer>
    <FlatList
      ...
      ListHeaderComponent={<View>
        <Pressable onPress={() => router.back()} style={styles.back}><MaterialIcon name="arrow-back" size={20} color="#062B5C" /><Text style={styles.backText}>Réglages</Text></Pressable>
        ...
        <View style={styles.exportRow}>
          <Pressable disabled={Boolean(exporting) || loading} onPress={() => void exportReport("csv")} style={({ pressed }) => [styles.exportButton, pressed && styles.pressed, (Boolean(exporting) || loading) && styles.disabled]}><MaterialIcon name="download" size={17} color="#003F87" /><Text style={styles.exportText}>{exporting === "csv" ? "Préparation…" : "Exporter CSV"}</Text></Pressable>
          <Pressable disabled={Boolean(exporting) || loading} onPress={() => void exportReport("pdf")} style={({ pressed }) => [styles.exportButton, pressed && styles.pressed, (Boolean(exporting) || loading) && styles.disabled]}><MaterialIcon name="picture-as-pdf" size={17} color="#003F87" /><Text style={styles.exportText}>{exporting === "pdf" ? "Préparation…" : "Exporter PDF"}</Text></Pressable>
        </View>
        ...
        {message ? <View style={styles.globalMessage}><MaterialIcon name="info-outline" size={17} color="#B4232B" /><Text style={styles.globalMessageText}>{message}</Text></View> : null}
        ...
        ].map((metric) => <View key={metric.label} style={styles.metric}><MaterialIcon name={metric.icon as never} size={18} color={metric.tone} /><Text style={styles.metricValue}>{metric.value}</Text><Text style={styles.metricLabel}>{metric.label}</Text></View>)}</View>
        <View style={styles.scopeNotice}><MaterialIcon name="admin-panel-settings" size={18} color="#062B5C" /><Text style={styles.scopeText}>Les exports reflètent exactement la période sélectionnée. Ils ne contiennent ni secret Meta, ni numéro client complet, ni donnée de paiement sensible.</Text></View>
      ...
      renderItem={({ item }) => {
        ...
        return <View style={[styles.agencyCard, { borderColor: `${color}66` }]}>
          ...
          <Pressable onPress={() => router.push({ pathname: "/agency-preview", params: { agencyId: item.agencyId } } as never)} style={({ pressed }) => [styles.agencyTop, pressed && styles.pressed]}>
            <View style={[styles.logo, { borderColor: color }]}>{item.logoUrl ? <Image source={{ uri: item.logoUrl }} style={styles.logoImage} /> : <MaterialIcon name="business" size={27} color={color} />}</View>
            ...
            <MaterialIcon name="arrow-forward" size={20} color={color} />
          </Pressable>
