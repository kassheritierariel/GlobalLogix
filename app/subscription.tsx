...
export default function SubscriptionScreen() {
  ...
  return <ScreenContainer className="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <Pressable onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcon name="arrow-back" size={20} color="#062B5C" /><Text style={styles.backText}>Réglages</Text></Pressable>
    ...
    <Pressable disabled={paying || loading} onPress={() => void openCheckout()} style={({ pressed }) => [styles.payButton, (paying || loading) && styles.disabled, pressed && styles.pressed]}>{paying ? <ActivityIndicator color="#062B5C" /> : <><MaterialIcon name="payments" size={20} color="#062B5C" /><Text style={styles.payButtonText}>Continuer vers le paiement Chariow</Text></>}</Pressable>
