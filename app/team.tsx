  ...
  const header = useMemo(() => (
    <View>
      <Pressable onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}>
        <MaterialIcon name="arrow-back" size={20} color="#0A2540" /><Text style={styles.backText}>Réglages</Text>
      </Pressable>
      ...
      {canManage ? <View style={styles.createBox}>
        ...
        <View style={styles.roleRow}>
          {(["staff", "viewer"] as const).map((itemRole) => <Pressable key={itemRole} onPress={() => setRole(itemRole)} style={({ pressed }) => [styles.roleButton, role === itemRole && styles.roleButtonSelected, pressed && styles.pressed]}><MaterialIcon name={itemRole === "staff" ? "build" : "visibility"} size={18} color={role === itemRole ? "#FFFFFF" : "#0A2540"} /><Text style={[styles.roleText, role === itemRole && styles.roleTextSelected]}>{itemRole === "staff" ? "Staff" : "Viewer"}</Text></Pressable>)}
        </View>
        <Pressable disabled={saving} onPress={() => void createMember()} style={({ pressed }) => [styles.createButton, (pressed || saving) && styles.pressed, saving && styles.disabled]}><MaterialIcon name="person-add" size={20} color="#FFFFFF" /><Text style={styles.createButtonText}>{saving ? "Traitement…" : "Créer et inviter"}</Text></Pressable>
      </View> : <View style={styles.denied}><MaterialIcon name="lock" size={24} color="#C43D3D" /><Text style={styles.deniedTitle}>Accès réservé</Text><Text style={styles.deniedText}>Seul l’administrateur de cette agence peut gérer les comptes staff et viewer.</Text></View>}
      {message ? <Text style={styles.message}>{message}</Text> : null}{error ? <Text style={styles.error}>{error}</Text> : null}
      {canManage ? <View style={teamSearchStyles.box}><MaterialIcon name="search" size={19} color="#718496" /><TextInput value={searchQuery} onChangeText={setSearchQuery} autoCapitalize="none" autoCorrect={false} placeholder="Rechercher par e-mail, rôle ou statut" placeholderTextColor="#718496" style={teamSearchStyles.input} /><Text style={teamSearchStyles.count}>{visibleMembers.length}/{members.length}</Text></View> : null}
